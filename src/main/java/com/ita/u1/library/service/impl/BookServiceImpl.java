...
public class BookServiceImpl implements BookService {
    ...
    public void changeCostPerDay(CopyBook copyBook) {
        ...
        if (!bookDAO.doesTheCopyBookExist(copyBook.getId())) {
    ...
    public void indicateBookViolation(ViolationBook violationBook) {
        serviceValidator.validateViolationMessage(violationBook);
        if (!bookDAO.doesTheCopyBookExist(violationBook.getCopyId())) {
            throw new ServiceException("There is no such book in the library.");
        }
        bookDAO.indicateBookViolation(violationBook);
    }
